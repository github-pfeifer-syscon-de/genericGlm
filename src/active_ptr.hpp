...
//        }
        ...
        {
            os << "lease addr " << std::hex << lease.get() << std::dec
               << " type " << typeid(lease.m_ptr).name()
               << " use " << (lease.m_active_ctl ? lease.m_active_ctl->get_usecount() : -1)
               << " lease " << (lease.m_active_ctl ? lease.m_active_ctl->get_leasecount() : -1);
            return os;
        }
    private:
    ...
    public:
        ...
        friend std::ostream& operator<<(std::ostream& os, const active_ptr<T>& ptr)
        {
            os << " addr " << std::hex << ptr.get() << std::dec
               << " type " << ((ptr.m_active_ctl) ? typeid(ptr.m_active_ctl->get_ptr()).name() : "null")
               << " use " << ptr.use_count()

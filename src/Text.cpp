...
Text::Text(GLenum type, NaviContext *ctx, Font *font)
: Geometry::Geometry(GL_TRIANGLES, ctx)
, m_font{font}
, m_shaderCtx{ctx}
, m_textCtx{nullptr}
, m_texID{0}
...
void
Text::setText(const Glib::ustring &text)
{
    m_text = text;
}
...
void Text::display(const Matrix &perspView)
{
    if (!m_visible) {
        return;
    }
...
void
Text::setTextContext(TextContext *textCtx)
{
    m_textCtx = textCtx;
}

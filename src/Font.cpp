...
FT_Face
Font::build(const std::string& path)
{
    FT_Face face;
...
std::string
Font::buildByName(const std::string &name)
{
...
Font::buildByGlyph(gunichar glyph)
{
    ...
    //std::cout << "Font::buildByGlyph glyph 0x" << std::hex << (int)glyph << std::dec << std::endl;
...
void
Font::debug(ShaderContext *ctx, Matrix &persView, Position &pos, GLfloat scale)
{
    // include ?

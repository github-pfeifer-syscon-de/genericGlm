...
FallShapeGeometry::FallShapeGeometry(Position& start, double size, TreeNode *treeNode, int depth, Font *font, GeometryContext *_ctx)
...
FallShapeGeometry::~FallShapeGeometry()
{
}

void
FallShapeGeometry::create(TreeNode *node, FallShapeRenderer &_renderer) {
    ...
    Color c0 = _renderer.pos2Color(m_start.x, node->getStage(), m_load);
    Color c2 = _renderer.pos2Color(end.x, node->getStage(), m_load);
...
bool
FallShapeGeometry::match(const TreeGeometry *treeGeo) {
    bool ret = FALSE;
    if(auto fallShape = dynamic_cast<const FallShapeGeometry*>(treeGeo)) {

        ret = fallShape->m_start == m_start &&
                fallShape->m_size == m_size &&
                fallShape->m_stage == m_stage &&
                fallShape->m_load == m_load;
//        if (!ret) {
